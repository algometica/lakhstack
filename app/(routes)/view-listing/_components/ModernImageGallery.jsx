...
function ModernImageGallery({ imageList }) {
  ...
  if (!imageList || imageList.length === 0) {
        return (
            <div className="bg-white rounded-3xl p-12 text-center border border-border/70 shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
                <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-secondary/70 flex items-center justify-center">
                    <Expand className="w-8 h-8 text-muted-foreground" />
                </div>
                <p className="text-muted-foreground text-lg">No images available for this listing</p>
            </div>
        )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative group">
                <div 
                    className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl bg-secondary/60 border border-border/70 shadow-[0_24px_50px_rgba(15,23,42,0.12)]"
          ...
          {imageList && imageList.length > 1 && (
            <>
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm border border-border/70 hover:bg-white opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg"
                                onClick={prevImage}
                            >
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm border border-border/70 hover:bg-white opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg"
                                onClick={nextImage}
                            >
          ...
          {/* Fullscreen Button */}
                    <Button
                        variant="ghost"
                        size="icon"
                        className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm border border-border/70 hover:bg-white opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg"
          ...
          {/* Image Counter */}
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 border border-border/70 shadow-lg">
